{
  SCNF Section File Editor

  This file contains everything to edit SCNF files tables.
  These sections contains subtitles used in Free Quest mode.

  It contains the proper method to modify these files, with a generic method.
  The concept of this engine and some parts of the code was re-used in newer
  version of Free Quest Editor, because this editor was made in 2008 with an
  improper hacking method
}
unit SCNFEdit;
...
uses
  SysTools,
  //LAZ: XMLDom, XMLIntf, MSXMLDom, XMLDoc,
  ActiveX, Variants;
...
function TSceneSectionsList.Add(F: TFileStream;
  SectionEntry: TSceneSectionHeader; AOffset: LongWord): Integer;
var
...
procedure TSceneSectionItem.AbsoluteSeek(F: TFileStream;
...
function TSceneSectionItem.Write(F: TFileStream): LongWord;
var
...
constructor TCharactersTable.Create(AOwner: TSceneSectionItem);
begin
  fList := TList.Create;
  fOwner := AOwner;
end;
...
function TSubtitlesList.Add(F: TFileStream;
  Entry: TSubtitleHeaderEntry): TSubtitlesListItem;
begin
...
constructor TSubtitlesList.Create(AOwner: TSceneSectionItem);
begin
  fList := TList.Create;
  fOwner := AOwner;
end;
...
constructor TSubtitlesListItem.Create(Owner: TSubtitlesList);
begin
  fOwner := Owner;
end;

procedure TSubtitlesListItem.DoParse(F: TFileStream);
var
  RawSub: string;
...
function TSubtitlesListItem.GetText: string;
begin
  Result := fText;
end;

procedure TSubtitlesListItem.SetText(const Value: string);
begin
